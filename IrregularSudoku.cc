...
bool IrregularSudoku::checkValidInRegion(int guess, std::vector<int>& region) {
...
bool IrregularSudoku::checkValidInRowCol(int guess, int row, int col, std::vector<std::vector<int>> &boardByRegion,
...
IrregularSudoku::IrregularSudoku(bool silent) {
    std::cin >> n;
    if (!silent) {
...
IrregularSudoku::IrregularSudoku(int n, std::vector<std::vector<int>> boardByRegion, std::vector<std::vector<std::pair<int, int>>> boardByRow,
...
IrregularSudoku::IrregularSudoku(const IrregularSudoku & other): n(other.n), boardByRegion(other.boardByRegion),
                boardByRow(other.boardByRow), regionToRowCol(other.regionToRowCol), filled(other.filled) {}

IrregularSudoku& IrregularSudoku::operator=(IrregularSudoku & other) {
    if (this == &other) return *this;
    IrregularSudoku tmp = IrregularSudoku(other);
    std::swap(n, tmp.n);
...
int IrregularSudoku::runDfs(int region, int jth) {
...
std::ostream& operator<<(std::ostream & outs, const IrregularSudoku & iso) {
